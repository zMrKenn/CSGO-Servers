...
void DB_ClanNameCheckCallback(Handle owner, Handle hndl, const char[] error, DataPack data)
{
	...
	else
	{
		...
		else	//Имя не занято
		{
			...
			FormatEx(query, sizeof(query), "INSERT INTO `clans_table` (`clan_id`, `clan_name`, `leader_steam`, `leader_name`, `time_creation`, `maxmembers`) VALUES ('%d', '%s', '%s', '%s', '%d', '%d');",
			clanid, clanNameEscaped, leaderAuth, leaderNameEscaped, creationTime, g_iStartSlotsInClan);
