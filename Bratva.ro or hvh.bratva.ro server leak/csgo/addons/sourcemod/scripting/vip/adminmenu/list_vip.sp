ShowVipPlayersListMenu(iClient)
{
	...
	*/
	...
	AddMenuItem(hMenu, "search", "Find a player\n \n");
	
	AddMenuItem(hMenu, "show_all", "Show All\n \n");
...
public MenuHandler_VipPlayersListMenu(Handle:hMenu, MenuAction:action, iClient, Item)
{
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			if(strcmp(sUserID, "more") == 0)	// show more
			{
			...
			if(strcmp(sUserID, "search") == 0)	// Find a player
			{
			...
			if(strcmp(sUserID, "show_all") == 0)	// Show All
			{
			...
			if(strncmp(sUserID, "uid_", 4) == 0)	// Temporary VIP status
			{
...
public SQL_Callback_SelectVipPlayers(Handle:hOwner, Handle:hQuery, const String:sError[], any:UserID)
{
	...
	if (iClient)
	{
		...
		else if(GetArrayCell(g_ClientData[iClient], DATA_OFFSET) != -1)
		{
			...
			AddMenuItem(hMenu, "more", "show more");
		}
