...
public Native_SetClientAccessTime(Handle:hPlugin, iNumParams)
{
	...
	if(CheckValidClient(iClient))
	{
		...
		{
			ThrowNativeError(SP_ERROR_NATIVE, "Invalid time (%i)", iTime);
			return false;
		}
	...
	if(CheckValidClient(iClient, false))
	{
		if(g_iClientInfo[iClient] & IS_VIP)
		{
			if(g_iClientInfo[iClient] & IS_AUTHORIZED)
			{
				...
				if(iClientID != -1)
				{
					ThrowNativeError(SP_ERROR_NATIVE, "The player %L is already a VIP", iClient, iClient);
					return false;
				}
			}
			else
		...
		if(UTIL_CheckValidVIPGroup(sGroup))
		{
			...
			if(iTime >= 0)
			{
				...
				if(bool:GetNativeCell(5) == true)
				{
					...
					{
						ThrowNativeError(SP_ERROR_NATIVE, "Invalid auth type (%i)", AuthType);
						return false;
					}
		...
		{
			ThrowNativeError(SP_ERROR_NATIVE, "Invalid VIP-group (%s)", sGroup);
		}
	}
...
public Native_RegisterFeature(Handle:hPlugin, iNumParams)
{
	...
	if(IsValidFeature(sFeatureName) == false)
	{
		...
		switch(FType)
		{
			...
			{
				SetArrayCell(hArray, FEATURES_COOKIE,		INVALID_HANDLE);
				SetArrayCell(hArray, FEATURES_ITEM_SELECT,	INVALID_HANDLE);
				SetArrayCell(hArray, FEATURES_ITEM_DISPLAY,	INVALID_HANDLE);
				SetArrayCell(hArray, FEATURES_ITEM_DRAW,	INVALID_HANDLE);
			}
		}
...
*/
	...
	{
		ThrowNativeError(SP_ERROR_NATIVE, "Feature \"%s\" already defined \"%s\" already exists", sFeatureName, sFeatureName);
	}
}
...
public Native_UnregisterFeature(Handle:hPlugin, iNumParams)
{
	...
	{
		ThrowNativeError(SP_ERROR_NATIVE, "Feature \"%s\" is invalid/Function \"%s\" does not exist", sFeatureName, sFeatureName);
	}
}
...
public Native_GiveClientFeature(Handle:hPlugin, iNumParams)
{
	...
	if(CheckValidClient(iClient))
	{
		...
		ThrowNativeError(SP_ERROR_NATIVE, "Invalid features (%s)", sFeatureName);
	}
...
bool:CheckValidClient(const &iClient, bool:bCheckVIP = true)
{
	...
	{	
		ThrowNativeError(SP_ERROR_NATIVE, "Client %i is not connected", iClient, iClient);
		return false;
	}
	if (bCheckVIP)
	{
